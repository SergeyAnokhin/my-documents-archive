...
| backend | [test_indexer.py](../backend/tests/test_indexer.py) | `_apply_analysis_result` writes all metadata columns, invalid-date handling, `_is_unclassified` predicate, `.docx`/`.txt` native-text branch (`_is_docx`/`_is_txt`/`_is_native_text`, `_run_docx_extract`/`_run_txt_extract` success + error contract) |
...
| backend | [test_text_extract.py](../backend/tests/test_text_extract.py) | Native `.txt` extraction: UTF-8 read-back, BOM stripping, latin-1 fallback for non-UTF-8 files, surrounding-whitespace trim |
