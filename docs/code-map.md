...
| `services/batch_ocr_gemini.py` | `run_batch_ocr_gemini()`: submit remote Gemini Batch job → poll → write OCR/analysis fields back, routing each document through `_needs_vision()` (vision vs text-only request). Imports `_needs_vision`/`_scope_filter`/`GEMINI_BATCH_BASE` from `batch_ocr.py`. `.docx`/`.txt` documents are extracted natively first (no page image exists), which makes `_needs_vision()` false so they fall into the existing text-only branch and still get analysis via the same batch job. Also the engine behind `task_runners._fix_quality()`'s analysis-gap fixes (`no_analysis`/`no_summary`/`no_tags`/`single_char_tag`/`no_category`), called with explicit `doc_ids`, so documents with no OCR/vision text yet still get tagged (one request: image in, text+tags out) instead of being silently skipped from a text-only batch. See [batch-ocr.md](batch-ocr.md) |
| `services/batch_analysis.py` | Gemini text-only Batch engine. Supports full analysis, `metadata_only`, and `classification_only`; the two reclassification tasks use classification-only scopes and preserve all metadata/manual types. Saves raw JSONL results under `.docintell/batch_results/`. Requires every selected document to already have `ocr_text`/`vision_description` — if none do, finishes `"done"` with `processed: 0` instead of uploading an empty batch (Gemini 400s on an empty `batchGenerateContent`); a batch-create HTTP error logs Gemini's response body plus model/request-count. For documents that may still lack any text, `_fix_quality()` uses `batch_ocr_gemini.py` instead (see above). See [batch-ocr.md § Empty batches and error visibility](batch-ocr.md#empty-batches-and-error-visibility) |
...
| `fix_quality` | Fix one quality gap (`no_ocr`/`no_embedding`/`no_analysis`/`no_summary`/`no_tags`/`single_char_tag`/`no_category`); analysis gaps are delegated to `run_batch_ocr_gemini` with explicit `doc_ids` so its hybrid vision/text routing sends the image (one request does recognition + analysis + tags) for documents with no OCR/vision text yet, and stays text-only for documents that already have text |
