...
| `services/batch_analysis.py` | Gemini text Batch engine. Supports full analysis, `metadata_only`, and `classification_only`; the two reclassification tasks use classification-only scopes and preserve all metadata/manual types. Saves raw JSONL results under `.docintell/batch_results/`. If every selected document has no usable text, finishes `"done"` with `processed: 0` instead of uploading an empty batch (Gemini 400s on an empty `batchGenerateContent`); a batch-create HTTP error logs Gemini's response body. See [batch-ocr.md § Empty batches and error visibility](batch-ocr.md#empty-batches-and-error-visibility) |
